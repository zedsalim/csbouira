const toggleDarkMode=()=>{document.documentElement.classList.toggle("dark"),document.querySelectorAll(".dark-mode-icon").forEach(e=>{e.classList.toggle("fa-moon"),e.classList.toggle("fa-sun")}),localStorage.setItem("darkMode",document.documentElement.classList.contains("dark"))};"true"===localStorage.getItem("darkMode")&&(document.documentElement.classList.add("dark"),document.querySelectorAll(".dark-mode-icon").forEach(e=>{e.classList.remove("fa-moon"),e.classList.add("fa-sun")}));const API_BASE="https://api.csbouira.xyz/api/drive",YEARS={licence:["Licence 1","Licence 2","Licence 3 SI"],master1:["Master 1 GSI","Master 1 ISIL","Master 1 IA"],master2:["Master 2 GSI","Master 2 ISIL","Master 2 IA"]},yearList=[...YEARS.licence,...YEARS.master1,...YEARS.master2];let currentPath=[],currentYear="",currentFiles=[],currentFileIndex=-1,onlineResources={};const loadYears=async()=>{try{renderYearCards(YEARS.licence,"licence-cards","fas fa-book-open"),renderYearCards(YEARS.master1,"master1-cards","fas fa-graduation-cap"),renderYearCards(YEARS.master2,"master2-cards","fas fa-user-graduate"),await loadAllFileCounts(),await loadOnlineResources()}catch(e){console.error("Error loading years:",e)}},renderYearCards=(e,t,a)=>{let s=document.getElementById(t);s.innerHTML=e.map(e=>`
      ...
      `).join("")},loadAllFileCounts=async()=>{try{let e=await fetch(`${API_BASE}?path=_fileCounts`),t=await e.json();if(!t||"object"!=typeof t){console.warn("No valid _fileCounts data received"),yearList.forEach(e=>showBadge(e,null));return}yearList.forEach(e=>{let a=t[e];showBadge(e,void 0!==a?a:null)})}catch(a){console.error("Error loading file counts:",a),yearList.forEach(e=>showBadge(e,null))}},loadOnlineResources=async()=>{try{let e=await fetch(`${API_BASE}?path=_onlineResources`);onlineResources=await e.json()}catch(t){console.error("Failed to load _onlineResources:",t),onlineResources={}}},showBadge=(e,t)=>{let a=document.getElementById(`badge-${e.replace(/\s+/g,"-").toLowerCase()}`);a&&(null==t?a.innerHTML=`<i class="fa-solid fa-file"></i> –`:a.innerHTML=`<i class="fa-solid fa-file"></i> ${t}`)},openYear=async(e,t)=>{t&&t.preventDefault(),currentYear=e,currentPath=[e];let a=document.getElementById("yearModalTitle");a.innerHTML=`
  ...
  `,document.getElementById("yearModal").classList.add("active"),updateBodyScrollLock(),await loadContent(e)},loadContent=async(e,t="")=>{let a=document.getElementById("yearContent");a.innerHTML=`
    <div class="text-center py-8">
      <div class="loading mx-auto"></div>
      <p class="mt-4">Loading...</p>
    </div>
  `;try{let s=`${API_BASE}?year=${encodeURIComponent(e)}`;t&&(s=`${API_BASE}?path=${encodeURIComponent(t)}`);let l=await fetch(s),n=await l.json(),i=document.getElementById("driveLink");n.link?(i.href=n.link,i.style.display="inline-flex",i.target="_blank",i.classList.add("active")):(i.style.display="none",i.classList.remove("active")),updateBreadcrumb(),renderContent(n),1===currentPath.length&&insertOnlineResources(e)}catch(r){a.innerHTML=`
    ...
    `;return}let a="";if(e.subfolders&&Object.keys(e.subfolders).length>0){a+=`
      <div class="mb-6">
        ...
        <div class="space-y-2">
    `;let s=Object.entries(e.subfolders);for(let[l]of s){let n=l.includes("(empty)"),i=l.replace(/'/g,"\\'").replace(/"/g,'\\"'),r={type:"folder",name:l,year:currentYear,path:[...currentPath,l].join(">subfolders>"),folderPath:[...currentPath,l]};a+=`
        <div class="folder-item ${n?"opacity-50":""}" ${n?"":`onclick="openFolder('${i}')"`}>
          <i class="fas fa-folder text-yellow-500"></i>
          <span class="flex-1">${l}</span>
          ${n?'<span class="text-sm">(empty)</span>':("function"==typeof createStarButton?createStarButton(r):"")+'<i class="fas fa-chevron-right"></i>'}
        </div>
      `}a+="</div></div>"}e.files&&e.files.length>0&&(currentFiles=e.files,a+=`
      <div>
        <h3 class="text-xl font-semibold mb-4 flex items-center gap-2">
          <i class="fas fa-file text-blue-500"></i>Files
        </h3>
        <div class="space-y-2">
    `,e.files.forEach(e=>{let t=getFileIcon(e.name),s=JSON.stringify(e).replace(/'/g,"\\'"),l={type:"file",name:e.name,year:currentYear,path:[...currentPath,e.name].join(">subfolders>"),folderPath:[...currentPath],link:e.link||"",previewLink:e.previewLink||"",downloadLink:e.downloadLink||""};a+=`
        <div class="file-item group">
          <div class="flex items-center gap-3 flex-1" onclick='openFile(${s})'>
            <i class="${t}"></i>
            <span class="flex-1">${e.name}</span>
            <i class="action-btn fas fa-eye"></i>
          </div>
          ${"function"==typeof createStarButton?createStarButton(l):""}
          <button 
      ...
      `}),a+="</div></div>"),t.innerHTML=a},insertOnlineResources=e=>{let t=onlineResources[e];if(!t)return;let a={};if(Array.isArray(t)?a.General=t:"object"==typeof t&&(a=t),0===Object.keys(a).length)return;let s=document.getElementById("yearContent"),l=`
    <div class="mt-8">
      <div class="mb-6">
