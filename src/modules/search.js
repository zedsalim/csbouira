...
function fuzzyMatch(text, token, maxDist = 2) {
  ...
  for (let i = 0; i <= textLower.length - wSize + maxDist; i++) {
    const substr = textLower.slice(
      i,
      Math.min(i + wSize + maxDist, textLower.length),
    );
...
function performSearch(query, filters) {
  ...
  let results = searchIndex.filter((item) => {
    ...
    if (
      ...
      !(item.resourceType || '')
        .toLowerCase()
        .includes(filters.resourceType.toLowerCase())
    )
      return false;
...
function triggerSearch() {
  ...
  for (const item of capped) {
    ...
    if (item.type === 'file' && tplFile) {
      ...
      if (item.downloadLink) {
        dlBtn.addEventListener('click', (e) => {
          ...
          window._downloadFile &&
            window._downloadFile(item.downloadLink, item.name);
        });
      } else {
      ...
      clone.querySelector('[data-field="badge"]').innerHTML = moduleBadge;
      clone
        .querySelector('[data-field="row"]')
        .addEventListener('click', () => {
          window._openSearchFolder &&
            window._openSearchFolder({
              year: item.year,
              path: item.path,
