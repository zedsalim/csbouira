...
async function startCamera() {
  ...
  try {
    ...
    if (rearCameras.length > 0) {
      const mainCamera =
        rearCameras.find((c) => c.getCapabilities?.().torch ?? false) ||
...
function updateTorchUI() {
  ...
  if (!icon) return;
  icon.className = torchOn
    ? 'fas fa-bolt text-yellow-400'
    : 'fas fa-bolt text-base-content/60';
}
...
function setupEditorEvents() {
  ...
  const onPointerDown = (e) => {
    ...
    if (activeTab === 'crop') {
      ...
      if (
        pos.x > cropState.x &&
        pos.x < cropState.x + cropState.w &&
        pos.y > cropState.y &&
        pos.y < cropState.y + cropState.h
      ) {
      ...
      for (let i = 0; i < 4; i++) {
        if (
          Math.hypot(
            pos.x - perspectiveState.points[i].x,
            pos.y - perspectiveState.points[i].y,
          ) < hT
        ) {
    ...
    if (activeTab === 'crop' && cropState.dragging) {
      ...
      if (cropState.handle === 'move') {
        ...
        cropState.x = Math.max(
          0,
          Math.min(editorCanvas.width - cropState.w, cropState._origX + dx),
        );
        cropState.y = Math.max(
          0,
          Math.min(editorCanvas.height - cropState.h, cropState._origY + dy),
        );
      } else {
      ...
      perspectiveState.points[i].x = Math.max(
        0,
        Math.min(editorCanvas.width, pos.x),
      );
      perspectiveState.points[i].y = Math.max(
        0,
        Math.min(editorCanvas.height, pos.y),
      );
      renderEditor();
    }
  };
  ...
  editorCanvas.addEventListener('touchstart', onPointerDown, {
    passive: false,
  });
...
function applyCrop() {
  ...
  const isFullImage =
    x <= 1 &&
    y <= 1 &&
    w >= editorCanvas.width - 2 &&
    h >= editorCanvas.height - 2;
  if (isFullImage) {
...
function applyPerspective() {
  ...
  const srcPts = perspectiveState.points.map((p) => ({
    x: p.x * scaleInvW,
    y: p.y * scaleInvH,
  }));
...
function applyColors() {
  ...
  const isDefault =
    colorState.brightness === 100 &&
    colorState.contrast === 100 &&
    colorState.saturation === 100 &&
    colorState.grayscale === 0 &&
    colorState.sharpness === 0 &&
    colorState.clarity === 0 &&
    colorState.highlights === 0;
  if (isDefault) {
...
function showToast(msg) {
  ...
  toast.className =
    'fixed bottom-6 left-1/2 -translate-x-1/2 bg-base-content text-base-100 px-4 py-2 rounded-lg text-sm font-medium z-[9999] shadow-lg transition-opacity';
...
async function saveScannerPdf() {
  ...
  if (src.width > maxDim || src.height > maxDim) {
    ...
    resized
      .getContext('2d')
      .drawImage(src, 0, 0, resized.width, resized.height);
    src = resized;
  }
